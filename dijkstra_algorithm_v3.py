...
### In this algorithm each time we check the neighbors of the vertex we are visiting, we pick the one that leads us closer to our budget goal.
### It fails when we run into a vertex that is not our destination and all its neighbors have already been visited.
...
def dijkstra(graph, src, dest, bud):
    # define infinity
    inf = sys.maxsize
    ...
    # create a list to record vertex that is dead end. (not destination, no neighbors we have not visited.)
    dead_end = []
    ...
    for i in range(2 * (len(node_data) - 1)):
        if current == dest:
        ...
        if current not in visited:
            ...
            for j in graph[current]:
                ...
                if j not in visited and j not in dead_end:
                    ...
                    if abs(bud - cost) < abs(bud - node_data[j]['cost']):
                    ...
                    heappush(min_heap, (abs(bud - node_data[j]["cost"]), j))
        ...
        # if we found no neighbor j in the j for loop to reach to.
        if not min_heap:
            # add current vertex to dead end list.
            dead_end.append(current)
            # and go back to previous current step.
            # taking that previous current step out of visited.
            current = visited.pop(-2)
        else:
    ...
    print("Vertices Visited: " + str(visited))
